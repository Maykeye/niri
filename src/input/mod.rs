...
impl State {
    ...
    // DEPRECATED: since better d-bus communication, this
    // work around wasn't needed
...
fn make_binds_iter<'a>(
    ...
    mods: Modifiers,
    keybinding_group: Option<usize>,
) -> impl Iterator<Item = &'a Bind> + Clone {
    let named_binds = &config.bind_groups;
    let active_bind_group = keybinding_group.and_then(|group_idx| named_binds.0.get(group_idx));

    // Figure out the binds to use depending on whether the MRU is enabled and/or open.
    let active_binds = active_bind_group.map(|x| x.binds.0.iter());
    let active_binds = active_binds.into_iter().flatten();
